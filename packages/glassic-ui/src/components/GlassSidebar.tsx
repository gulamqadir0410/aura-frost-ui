...
const GlassSidebar = React.forwardRef<HTMLDivElement, GlassSidebarProps>(
  ({
    className, groups = [], collapsed: controlledCollapsed, onCollapsedChange,
    pinned = false, onPinnedChange, width = 260, collapsedWidth = 64, hoverExpand = false, ...props
  }, ref) => {
    ...
    return (
      <div
        ...
        className={cn("h-full glass-2 border-r border-border flex flex-col overflow-hidden", className)}
        {...props}
      >
        <div className="flex items-center justify-between p-3 border-b border-border">
          ...
          <div className="flex items-center gap-1 ml-auto">
            {onPinnedChange && showExpanded && (
              <button onClick={() => onPinnedChange(!pinned)} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors" aria-label={pinned ? "Unpin sidebar" : "Pin sidebar"}>
                {pinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />}
              </button>
            )}
            <button onClick={() => setCollapsed(!isCollapsed)} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors" aria-label={isCollapsed ? "Expand sidebar" : "Collapse sidebar"}>
          ...
          {groups.map((group, i) => (
            <div key={group.label || i}>
              {group.label && showExpanded && (
                <div className="px-3 py-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground/70">{group.label}</div>
              )}
